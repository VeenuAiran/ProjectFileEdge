...
public class InboundMgmtWorker extends Thread {
	...
	@Override
	public void run() {
		System.out.println("Inside the InboundMgmtWorker.run()...");
		while (true) {
			...
			try {
				...
				System.out.println("Inboud Message recieved...");
				logger.info("Inbound message received");
				Management req = (Management) msg.req;
				if (req.hasBeat()) {
					System.out.println("Inside if(req.hasBeat())...");
					/**
		...
		System.out.println("Exiting the InboundMgmtWorker.run()...");
	}
}
