...
public class HeartbeatManager extends Thread {
	...
	public void updateInboundHB(String nodeId) {
		System.out.println("Inside the HeartbeatManager.updateInboundHB()... Updating with :: "+nodeId);
		...
		if (hd == null) {
			...
			System.out.println("Exiting the HeartbeatManager.updateInboundHB()...");
		}
	}
	...
	protected void addNearestNode(HeartbeatData node) {
		System.out.println("Entered the HeartBeatManager.addNearestNode()...");
		...
		if (!incomingHB.containsKey(node.getNodeId())) {
			System.out.println("Checked if :: "+node.getNodeId()+" is present in the Incoming Concurrent Hash Map...");
		...
		System.out.println("Exiting the HeartBeatManager.addNearestNode()...");
	}
	...
	@Override
	public void run() {
		System.out.println("Entering the HeartbeatManager.run()...");
		...
		while (forever) {
			try {
				...
				if (outgoingHB.size() > 0) {
					System.out.println("We have the edge with a new node now...");
					...
					; // logger.info("No nodes to send HB"); (can be something here)
		...
		System.out.println("Exiting the HeartbeatManager.run()...");
	}
